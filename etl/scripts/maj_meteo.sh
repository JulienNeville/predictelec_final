...
LOG_FILE="$LOG_DIR/maj_meteo.log"
...
echo "----------------------------------------" >> "$LOG_FILE"
echo "Début MAJ_METEO : $(date)" >> "$LOG_FILE"

# Exécution du job 
# ajout --no-deps pour ne pas relancer postgres
docker compose -f "$PROJECT_DIR/docker-compose.yml" run --rm --no-deps app MAJ_METEO >> "$LOG_FILE" 2>&1
EXIT_CODE=$?
...
if [ $EXIT_CODE -ne 0 ]; then
	#on log l'erreur
	echo "Erreur MAJ_METEO le $END_TIME : code=$EXIT_CODE" >> "$LOG_FILE"
	#envoie de l'erreur
    if [ -z "$ALERT_EMAIL" ]; then
        echo "ALERT_EMAIL non définie !" >> "$LOG_FILE"
    else
        echo "Erreur MAJ_METEO le $END_TIME" | mail -s "ERREUR PREDICTELEC MAJ_METEO" "$ALERT_EMAIL"
    fi
else
	echo "Fin MAJ_METEO : $END_TIME" >> "$LOG_FILE"
fi
